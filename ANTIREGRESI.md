...
- **[v0.9.1] Dropdown filter berbasis enum HARUS selalu diisi dinamis dari data asli, tidak boleh daftar tebakan statis di HTML.** Bug ini muncul DUA KALI dengan pola sama: pertama di `guru/bank-soal.html` (v0.9.0), lalu ditemukan lagi di `guru/susun-paket.html` (v0.9.1) karena perbaikan pertama tidak ikut diterapkan ke halaman lain yang punya dropdown serupa. Kalau menambah dropdown filter baru berbasis field Firestore apa pun (bukan cuma kompleksitas) di halaman mana pun, isi opsinya dengan `[...new Set(data.map(...))]` dari hasil query, JANGAN menulis daftar opsi tebakan langsung di HTML.
